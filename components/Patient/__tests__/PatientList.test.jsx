import React from 'react';
import { render, screen } from '@testing-library/react-native';
import PatientList from '../PatientList';

const STUB_ITEMS = [
    {
    	id: 1,
    	timeVisit: '11:30',
    	visitReason: 'cleaning and planning review',
    	active: true,
    	userInfo: {
    		fullName: 'Piotr Rudnicky',
    		avatar:
    			'data:image/jpeg;base64,',
    		phoneNumber: '+4851545511111',
    	},
    },
];

describe('<PatientList /> component', () => {
	it('should be rendered with current data', () => {
		const { debug, getByText } = render(<PatientList items={STUB_ITEMS} />);
        const fullName = getByText('Piotr Rudnicky');
        const visitTime = getByText('11:30');
        const reasonVisit = getByText('cleaning and planning review');
        expect(fullName).toBeTruthy();
        expect(visitTime).toBeTruthy();
        expect(reasonVisit).toBeTruthy();
	});
});
